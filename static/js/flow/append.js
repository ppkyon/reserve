    ...
    if ( data.type == 1 ) {
        if ( 'id' in data.template ) {
            if ( data.template.message_type == 1 ) {
                ...
                html += '<a href="/company/template/greeting/" target="_blank">';
        ...
        if ( check_empty( data.template ) ) {
            if ( data.template.type == 1 ) {
                ...
                html += '<div class="chart-content position-relative mb-4">';
                html += '<a href="/company/template/cardtype/edit/?id=' + data.template.display_id + '" target="_blank">';
        ...
        html += '</div>';
        if ( check_empty(name) ) {
            html += '<a href="/company/richmenu/edit/?id=' + display_id + '" target=_blank>';
...
function append_table_area(data) {
    var favorite = '<td>';
    favorite += '<form id="favorite_form" action="/flow/favorite/" method="POST" enctype="multipart/form-data" data-parsley-focus="none">';
    favorite += '<input type="hidden" name="id" value="' + data.display_id + '">';
    favorite += '</form>';
    if ( data.favorite_flg ) {
        favorite += '<img class="favorite-icon d-none" src="' + $( '#env_static_url' ).val() + 'img/icon/star.svg">';
        favorite += '<img class="favorite-icon" src="' + $( '#env_static_url' ).val() + 'img/icon/star-color.svg">';
    } else {
        favorite += '<img class="favorite-icon" src="' + $( '#env_static_url' ).val() + 'img/icon/star.svg">';
        favorite += '<img class="favorite-icon d-none" src="' + $( '#env_static_url' ).val() + 'img/icon/star-color.svg">';
    }
    favorite += '<button type="button" class="d-none" data-bs-toggle="modal" data-bs-target="#favorite_on_modal"></button>';
    favorite += '<button type="button" class="d-none" data-bs-toggle="modal" data-bs-target="#favorite_off_modal"></button>';
    favorite += '</td>';

    var button = '<a href="/flow/edit/?id=' + data.display_id + '" class="btn detail-button p-1">詳細</a>';
