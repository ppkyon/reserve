...
$( function() {
    ...
    $( document ).on( 'click', '.table-area .genre-table tbody tr', function () {
        if ( !$( this ).hasClass( 'active' ) ) {
            ...
            form_data.append( 'text', $( '.table-control-area .tag-search' ).val() );
    ...
    $( document ).on( 'click', '.table-area .genre-table .dropdown .delete-button', function () {
        ...
        var html = '<div class="modal-description-area ps-3 pe-3">';
        html += '<p class="modal-description mb-0">このジャンルに属するタグもすべて削除されます。</p>';
        html += '</div>';
        $( '#delete_check_modal .modal-body .col-12' ).append( html );
    });
