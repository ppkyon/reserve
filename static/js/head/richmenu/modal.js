function open_rich_menu_modal( target ) {
    ...
    $.ajax({
        'data': form_data,
        'url': $( '#get_head_rich_menu_list_url' ).val(),
        'type': 'POST',
        'dataType': 'json',
        'processData': false,
        'contentType': false,
    }).done( function( response ){
        $( '#head_rich_menu_modal .table-area tbody' ).empty();
        if ( response.length > 0 ) {
            $.each( response, function( index, value ) {
                $( '#head_rich_menu_modal .table-area tbody' ).append( append_rich_menu_modal( value ) );
            });
            $( '#head_rich_menu_modal .table-area' ).removeClass( 'd-none' );
            $( '#head_rich_menu_modal .notice-area' ).addClass( 'd-none' );
        } else {
            $( '#head_rich_menu_modal .table-area' ).addClass( 'd-none' );
            $( '#head_rich_menu_modal .notice-area' ).removeClass( 'd-none' );
        }
