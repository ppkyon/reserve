function open_template_cardtype_modal(target, number){
    ...
    $.ajax({
        'data': form_data,
        'url': $( '#get_company_template_cardtype_list_url' ).val(),
        'type': 'POST',
        'dataType': 'json',
        'processData': false,
        'contentType': false,
    }).done( function( response ){
        $( '#company_template_cardtype_modal .table-area tbody' ).empty();
        if ( response.length > 0 ) {
            $.each( response, function( index, value ) {
                $( '#company_template_cardtype_modal .table-area tbody' ).append( append_template_cardtype_modal(value) );
            });
            $( '#company_template_cardtype_modal .table-area tbody button' ).val( number );
            $( '#company_template_cardtype_modal .table-area' ).removeClass( 'd-none' );
            $( '#company_template_cardtype_modal .notice-area' ).addClass( 'd-none' );
        } else {
            $( '#company_template_cardtype_modal .table-area' ).addClass( 'd-none' );
            $( '#company_template_cardtype_modal .notice-area' ).removeClass( 'd-none' );
        }
