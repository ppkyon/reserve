function open_template_video_modal(target, number){
    ...
    $.ajax({
        'data': form_data,
        'url': $( '#get_company_template_video_list_url' ).val(),
        'type': 'POST',
        'dataType': 'json',
        'processData': false,
        'contentType': false,
    }).done( function( response ){
        $( '#company_template_video_modal .table-area tbody' ).empty();
        if ( response.length > 0 ) {
            $.each( response, function( index, value ) {
                if ( check_empty( value.video ) ) {
                    $( '#company_template_video_modal .table-area tbody' ).append( append_template_video_modal(value) );
                }
            });
            $( '#company_template_video_modal .table-area tbody button' ).val( number );
            $( '#company_template_video_modal .table-area' ).removeClass( 'd-none' );
            $( '#company_template_video_modal .notice-area' ).addClass( 'd-none' );
        } else {
            $( '#company_template_video_modal .table-area' ).addClass( 'd-none' );
            $( '#company_template_video_modal .notice-area' ).removeClass( 'd-none' );
        }
