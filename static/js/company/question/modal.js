function open_question_modal(target, number){
    ...
    $.ajax({
        'data': form_data,
        'url': $( '#get_company_question_list_url' ).val(),
        'type': 'POST',
        'dataType': 'json',
        'processData': false,
        'contentType': false,
    }).done( function( response ){
        $( '#company_question_modal .table-area tbody' ).empty();
        if ( response.length > 0 ) {
            $.each( response, function( index, value ) {
                $( '#company_question_modal .table-area tbody' ).append( append_question_data(value) );
            });
            $( '#company_question_modal .table-area tbody button' ).val( number );
            $( '#company_question_modal .table-area' ).removeClass( 'd-none' );
            $( '#company_question_modal .notice-area' ).addClass( 'd-none' );
        } else {
            $( '#company_question_modal .table-area' ).addClass( 'd-none' );
            $( '#company_question_modal .notice-area' ).removeClass( 'd-none' );
        }
