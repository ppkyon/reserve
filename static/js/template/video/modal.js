function open_template_video_modal(target, number){
    ...
    $.ajax({
        'data': form_data,
        'url': $( '#get_template_video_list_url' ).val(),
        'type': 'POST',
        'dataType': 'json',
        'processData': false,
        'contentType': false,
    }).done( function( response ){
        $( '#template_video_modal .table-area tbody' ).empty();
        if ( response.length > 0 ) {
            $.each( response, function( index, value ) {
                if ( check_empty( value.video ) ) {
                    $( '#template_video_modal .table-area tbody' ).append( append_template_video_modal(value) );
                }
            });
