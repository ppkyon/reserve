function open_rich_menu_modal( target ) {
    ...
    $.ajax({
        'data': form_data,
        'url': $( '#get_rich_menu_list_url' ).val(),
        'type': 'POST',
        'dataType': 'json',
        'processData': false,
        'contentType': false,
    }).done( function( response ){
        $( '#rich_menu_modal .table-area tbody' ).empty();
        if ( response.length > 0 ) {
            $.each( response, function( index, value ) {
                $( '#rich_menu_modal .table-area tbody' ).append( append_rich_menu_modal( value ) );
            });
            $( '#rich_menu_modal .table-area' ).removeClass( 'd-none' );
            $( '#rich_menu_modal .notice-area' ).addClass( 'd-none' );
        } else {
            $( '#rich_menu_modal .table-area' ).addClass( 'd-none' );
            $( '#rich_menu_modal .notice-area' ).removeClass( 'd-none' );
        }
