...
def save(request):
    ...
    if request.POST.get('name'):
        if request.POST.get('id'):
            tag_genre = ShopTagGenre.objects.filter(display_id=request.POST.get('id')).first()
            tag_genre.name = request.POST.get('name')
            tag_genre.save()
        else:
            tag_genre = ShopTagGenre.objects.create(
                id = str(uuid.uuid4()),
                display_id = create_code(12, ShopTagGenre),
                shop = auth_login.shop,
                name = request.POST.get('name'),
                count = 0,
            )
        count = ShopTag.objects.filter(genre=tag_genre).count()
        return JsonResponse( {'id': tag_genre.display_id, 'count': count}, safe=False )

    return JsonResponse( {'id': ''}, safe=False )

def delete(request):
    ShopTagGenre.objects.filter(display_id=request.POST.get('id')).first().delete()
