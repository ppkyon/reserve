...
def get(request):
    ...
    question['item'] = list(UserQuestionItem.objects.filter(question__id=question['id']).values(*get_model_field(UserQuestionItem)).all())
    for question_index, question_item in enumerate(question['item']):
        question['item'][question_index]['choice'] = list(UserQuestionItemChoice.objects.filter(question__id=question_item['id']).values(*get_model_field(UserQuestionItemChoice)).all())
