...
def get_list(request, page):
    ...
    if request.POST.get('item') == 'line':
        ...
        if sort:
            if sort.target == 'name':
                if sort.sort == 1:
                    data = list(LineUser.objects.annotate(new=Subquery(new.values('created_at')[:1])).filter(shop=auth_login.shop, member_flg=False, proxy_flg=False, check_flg=False, delete_flg=False).order_by('new', 'user_profile__name_kana', 'created_at').values(*get_model_field(LineUser)).distinct().all()[start:end])
                elif sort.sort == 2:
                    data = list(LineUser.objects.annotate(new=Subquery(new.values('created_at')[:1])).filter(shop=auth_login.shop, member_flg=False, proxy_flg=False, check_flg=False, delete_flg=False).order_by('new', '-user_profile__name_kana', 'created_at').values(*get_model_field(LineUser)).distinct().all()[start:end])
                else:
                    data = list(LineUser.objects.annotate(new=Subquery(new.values('created_at')[:1])).filter(shop=auth_login.shop, member_flg=False, proxy_flg=False, check_flg=False, delete_flg=False).order_by('new', 'created_at').values(*get_model_field(LineUser)).distinct().all()[start:end])
            else:
                if sort.sort == 1:
                    data = list(LineUser.objects.annotate(new=Subquery(new.values('created_at')[:1])).filter(shop=auth_login.shop, member_flg=False, proxy_flg=False, check_flg=False, delete_flg=False).order_by('new', 'created_at').values(*get_model_field(LineUser)).distinct().all()[start:end])
                elif sort.sort == 2:
