        ...
        if context['today_reserve_sort']:
            if context['today_reserve_sort'].target == 'name':
                if context['today_reserve_sort'].sort == 1:
                    today_reserve_list = UserFlowSchedule.objects.filter(query, flow__user__shop=auth_login.shop, date=now.replace(hour=0, minute=0, second=0, microsecond=0), date__isnull=False, temp_flg=False).exclude(Q(number=0)|Q(join=2)).order_by('flow__user__user_profile__name_kana', 'date', 'time', 'flow__flow_tab__number', '-created_at').all()[:context['today_reserve_table']['number']]
                elif context['today_reserve_sort'].sort == 2:
                    today_reserve_list = UserFlowSchedule.objects.filter(query, flow__user__shop=auth_login.shop, date=now.replace(hour=0, minute=0, second=0, microsecond=0), date__isnull=False, temp_flg=False).exclude(Q(number=0)|Q(join=2)).order_by('-flow__user__user_profile__name_kana', 'date', 'time', 'flow__flow_tab__number', '-created_at').all()[:context['today_reserve_table']['number']]
                else:
                    today_reserve_list = UserFlowSchedule.objects.filter(query, flow__user__shop=auth_login.shop, date=now.replace(hour=0, minute=0, second=0, microsecond=0), date__isnull=False, temp_flg=False).exclude(Q(number=0)|Q(join=2)).order_by('date', 'time', 'flow__flow_tab__number', '-created_at').all()[:context['today_reserve_table']['number']]
                ...
                if context['today_reserve_sort'].sort == 1:
                    today_reserve_list = UserFlowSchedule.objects.filter(query, flow__user__shop=auth_login.shop, date=now.replace(hour=0, minute=0, second=0, microsecond=0), date__isnull=False, temp_flg=False).exclude(Q(number=0)|Q(join=2)).order_by('flow__user__proxy_flg', 'date', 'time', 'flow__flow_tab__number', '-created_at').all()[:context['today_reserve_table']['number']]
                elif context['today_reserve_sort'].sort == 2:
                    today_reserve_list = UserFlowSchedule.objects.filter(query, flow__user__shop=auth_login.shop, date=now.replace(hour=0, minute=0, second=0, microsecond=0), date__isnull=False, temp_flg=False).exclude(Q(number=0)|Q(join=2)).order_by('-flow__user__proxy_flg', 'date', 'time', 'flow__flow_tab__number', '-created_at').all()[:context['today_reserve_table']['number']]
                else:
                    today_reserve_list = UserFlowSchedule.objects.filter(query, flow__user__shop=auth_login.shop, date=now.replace(hour=0, minute=0, second=0, microsecond=0), date__isnull=False, temp_flg=False).exclude(Q(number=0)|Q(join=2)).order_by('date', 'time', 'flow__flow_tab__number', '-created_at').all()[:context['today_reserve_table']['number']]
        ...
        if context['after_reserve_sort']:
            if context['after_reserve_sort'].target == 'name':
                if context['after_reserve_sort'].sort == 1:
                    after_reserve_list = UserFlowSchedule.objects.filter(query, flow__user__shop=auth_login.shop, date__gte=after.replace(hour=0, minute=0, second=0, microsecond=0), date__isnull=False, temp_flg=False).exclude(Q(number=0)|Q(join=2)).order_by('flow__user__user_profile__name_kana', 'date', 'time', 'flow__flow_tab__number', '-created_at').all()[:context['after_reserve_table']['number']]
                elif context['after_reserve_sort'].sort == 2:
                    after_reserve_list = UserFlowSchedule.objects.filter(query, flow__user__shop=auth_login.shop, date__gte=after.replace(hour=0, minute=0, second=0, microsecond=0), date__isnull=False, temp_flg=False).exclude(Q(number=0)|Q(join=2)).order_by('-flow__user__user_profile__name_kana', 'date', 'time', 'flow__flow_tab__number', '-created_at').all()[:context['after_reserve_table']['number']]
                else:
                    after_reserve_list = UserFlowSchedule.objects.filter(query, flow__user__shop=auth_login.shop, date__gte=after.replace(hour=0, minute=0, second=0, microsecond=0), date__isnull=False, temp_flg=False).exclude(Q(number=0)|Q(join=2)).order_by('date', 'time', 'flow__flow_tab__number', '-created_at').all()[:context['after_reserve_table']['number']]
                ...
                if context['after_reserve_sort'].sort == 1:
                    after_reserve_list = UserFlowSchedule.objects.filter(query, flow__user__shop=auth_login.shop, date__gte=after.replace(hour=0, minute=0, second=0, microsecond=0), date__isnull=False, temp_flg=False).exclude(Q(number=0)|Q(join=2)).order_by('flow__user__proxy_flg', 'date', 'time', 'flow__flow_tab__number', '-created_at').all()[:context['after_reserve_table']['number']]
                elif context['after_reserve_sort'].sort == 2:
                    after_reserve_list = UserFlowSchedule.objects.filter(query, flow__user__shop=auth_login.shop, date__gte=after.replace(hour=0, minute=0, second=0, microsecond=0), date__isnull=False, temp_flg=False).exclude(Q(number=0)|Q(join=2)).order_by('-flow__user__proxy_flg', 'date', 'time', 'flow__flow_tab__number', '-created_at').all()[:context['after_reserve_table']['number']]
                else:
                    after_reserve_list = UserFlowSchedule.objects.filter(query, flow__user__shop=auth_login.shop, date__gte=after.replace(hour=0, minute=0, second=0, microsecond=0), date__isnull=False, temp_flg=False).exclude(Q(number=0)|Q(join=2)).order_by('date', 'time', 'flow__flow_tab__number', '-created_at').all()[:context['after_reserve_table']['number']]
        ...
        if context['new_line_sort']:
            if context['new_line_sort'].target == 'name':
                if context['new_line_sort'].sort == 1:
                    new_line_list = LineUser.objects.annotate(new=Subquery(new.values('check_flg')[:1])).filter(shop=auth_login.shop, member_flg=False, proxy_flg=False, check_flg=False, delete_flg=False).order_by('new', 'user_profile__name_kana', 'created_at').all()[:context['new_line_table']['number']]
                elif context['new_line_sort'].sort == 2:
                    new_line_list = LineUser.objects.annotate(new=Subquery(new.values('check_flg')[:1])).filter(shop=auth_login.shop, member_flg=False, proxy_flg=False, check_flg=False, delete_flg=False).order_by('new', '-user_profile__name_kana', 'created_at').all()[:context['new_line_table']['number']]
                else:
                    new_line_list = LineUser.objects.annotate(new=Subquery(new.values('check_flg')[:1])).filter(shop=auth_login.shop, member_flg=False, proxy_flg=False, check_flg=False, delete_flg=False).order_by('new', 'created_at').all()[:context['new_line_table']['number']]
            else:
                if context['new_line_sort'].sort == 1:
                    new_line_list = LineUser.objects.annotate(new=Subquery(new.values('check_flg')[:1])).filter(shop=auth_login.shop, member_flg=False, proxy_flg=False, check_flg=False, delete_flg=False).order_by('new', 'created_at').all()[:context['new_line_table']['number']]
                elif context['new_line_sort'].sort == 2:
