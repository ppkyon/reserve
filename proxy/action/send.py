...
def send(request):
    ...
    if not UserFlowSchedule.objects.filter(temp_manager=auth_login.user, number=0, temp_flg=True).exists():
    ...
    if ReserveOfflineSetting.objects.filter(display_id=request.POST.get('setting_id')).exists():
        ...
        for menu in ReserveOfflineFlowMenu.objects.filter(shop=auth_login.shop, offline=setting).all():
            flow_tab = ShopFlowTab.objects.filter(flow__shop=auth_login.shop, name=menu.flow).first()
