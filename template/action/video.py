...
from template.action.list import get_video_list
...
def save(request):
    ...
    if request.POST.get('id') and ShopTemplateVideo.objects.filter(display_id=request.POST.get('id')).exists():
        template = ShopTemplateVideo.objects.filter(display_id=request.POST.get('id')).first()
        template.name = request.POST.get('name')
        template.favorite_flg = favorite_flg
        template.author = request.user.id
        template.save()
    else:
        template = ShopTemplateVideo.objects.create(
            id = str(uuid.uuid4()),
...
def delete(request):
    ShopTemplateVideo.objects.filter(display_id=request.POST.get('id')).all().delete()
    return JsonResponse( {}, safe=False )
...
def copy(request):
    return JsonResponse( {'copy_url': reverse('template:edit_video') + '?copy=' + request.POST.get('id')}, safe=False )
...
def get(request):
    template = ShopTemplateVideo.objects.filter(display_id=request.POST.get('id')).values(*get_model_field(ShopTemplateVideo)).first()
    template['video_display_time'] = time.strftime('%M:%S', time.gmtime(template['video_time']))
    return JsonResponse( template, safe=False )

def get_all(request):
    auth_login = AuthLogin.objects.filter(user=request.user).first()
    template_list = list(ShopTemplateVideo.objects.filter(company=auth_login.shop.company, shop=auth_login.shop).order_by('-created_at').values(*get_model_field(ShopTemplateVideo)).all())
