...
from company.template.action.list import get_video_list
...
def save(request):
    ...
    if request.POST.get('id') and CompanyTemplateVideo.objects.filter(display_id=request.POST.get('id')).exists():
        template = CompanyTemplateVideo.objects.filter(display_id=request.POST.get('id')).first()
        template.name = request.POST.get('name')
        template.author = request.user.id
        template.save()
    else:
        template = CompanyTemplateVideo.objects.create(
            id = str(uuid.uuid4()),
...
def delete(request):
    CompanyTemplateVideo.objects.filter(display_id=request.POST.get('id')).all().delete()
    return JsonResponse( {}, safe=False )

def copy(request):
    return JsonResponse( {'copy_url': reverse('company:template:edit_video') + '?copy=' + request.POST.get('id')}, safe=False )
...
def get(request):
    template = CompanyTemplateVideo.objects.filter(display_id=request.POST.get('id')).values(*get_model_field(CompanyTemplateVideo)).first()
    template['video_display_time'] = time.strftime('%M:%S', time.gmtime(template['video_time']))
    return JsonResponse( template, safe=False )

def get_all(request):
    auth_login = AuthLogin.objects.filter(user=request.user).first()
    template_list = list(CompanyTemplateVideo.objects.filter(company=auth_login.company).order_by('-created_at').values(*get_model_field(CompanyTemplateVideo)).all())
