...
from template.models import CompanyTemplateRichVideo

from company.template.action.list import get_richvideo_list
...
def save(request):
    ...
    if request.POST.get('id') and CompanyTemplateRichVideo.objects.filter(display_id=request.POST.get('id')).exists():
        template = CompanyTemplateRichVideo.objects.filter(display_id=request.POST.get('id')).first()
        template.name = request.POST.get('name')
        template.title = request.POST.get('title')
        template.author = request.user.id
        template.save()
    else:
        template = CompanyTemplateRichVideo.objects.create(
            id = str(uuid.uuid4()),
            display_id = create_code(12, CompanyTemplateRichVideo),
...
def delete(request):
    CompanyTemplateRichVideo.objects.filter(display_id=request.POST.get('id')).all().delete()
    return JsonResponse( {}, safe=False )

def copy(request):
    return JsonResponse( {'copy_url': reverse('company:template:edit_richvideo') + '?copy=' + request.POST.get('id')}, safe=False )
...
def get(request):
    template = CompanyTemplateRichVideo.objects.filter(display_id=request.POST.get('id')).values(*get_model_field(CompanyTemplateRichVideo)).first()
    template['video_display_time'] = time.strftime('%M:%S', time.gmtime(template['video_time']))
    return JsonResponse( template, safe=False )

def get_all(request):
    auth_login = AuthLogin.objects.filter(user=request.user).first()
    template_list = list(CompanyTemplateRichVideo.objects.filter(company=auth_login.company).order_by('-created_at').values(*get_model_field(CompanyTemplateRichVideo)).all())
