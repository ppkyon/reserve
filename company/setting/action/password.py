...
from sign.models import AuthUser, PasswordChangeToken
...
def change(request):
    ...
    template = get_template('company/setting/email/change_password.txt')
...
def reset(request):
    ...
    template = get_template('company/setting/email/reset_password.txt')
    site = get_current_site(request)
    context = {
        'protocol': 'https' if request.is_secure() else 'http',
        'domain': site.domain,
