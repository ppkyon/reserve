...
from sign.models import AuthLogin, AuthCompany, CompanyProfile
...
def get_profile(request):
    ...
    company = AuthCompany.objects.filter(id=auth_login.company.id).values(*get_model_field(AuthCompany)).first()
    company['profile'] = CompanyProfile.objects.filter(company=auth_login.company).values(*get_model_field(CompanyProfile)).first()
    ...
    tag_list = list()
    for company_tag_item in CompanyHashTag.objects.filter(company=auth_login.company).order_by('number').all():
