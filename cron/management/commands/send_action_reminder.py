...
from line.action.message import push_text_message, push_image_message, push_video_message
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        remove = re.compile(r"<[^>]*?>")
        send_flg = True
        for reminder_item in UserFlowActionReminder.objects.filter(action_date__lte=datetime.datetime.now()).all():
            global line_bot_api
            ...
            if reminder_item.template_text:
                for template_text_item in ShopTemplateTextItem.objects.filter(template=reminder_item.template_text).all():
                    print(template_text_item.message_type)
