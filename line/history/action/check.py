...
def check(request):
    ...
    for user_flow_index, user_flow_item in enumerate(user_flow):
        user_flow[user_flow_index]['schedule'] = UserFlowSchedule.objects.filter(flow__id=user_flow_item['id']).order_by('-number').values(*get_model_field(UserFlowSchedule)).first()
        if user_flow[user_flow_index]['schedule']:
            ...
            if user_flow[user_flow_index]['schedule']['online']:
                ...
                user_flow[user_flow_index]['shop'] = ShopOnline.objects.filter(id=user_flow[user_flow_index]['setting']['online']).values(*get_model_field(ShopOnline)).first()
                ...
                add_date = date + datetime.timedelta(minutes=user_flow[user_flow_index]['setting']['time'])
                user_flow[user_flow_index]['shop'] = ShopOffline.objects.filter(id=user_flow[user_flow_index]['setting']['offline']).values(*get_model_field(ShopOffline)).first()
    ...
    for user_question_index, user_question_item in enumerate(user_question):
        ...
        for user_question_item_index, user_question_item_item in enumerate(user_question[user_question_index]['item']):
            user_question[user_question_index]['item'][user_question_item_index]['data'] = ShopQuestionItem.objects.filter(id=user_question_item_item['question']).values(*get_model_field(ShopQuestionItem)).first()
            ...
            for user_question_item_choice_index, user_question_item_choice_item in enumerate(user_question[user_question_index]['item'][user_question_item_index]['choice']):
                user_question[user_question_index]['item'][user_question_item_index]['choice'][user_question_item_choice_index]['data'] = ShopQuestionItemChoice.objects.filter(id=user_question_item_choice_item['question']).values(*get_model_field(ShopQuestionItemChoice)).first()
